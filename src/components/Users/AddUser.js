...
const AddUser = ({ closeModal, settabValue }) => {
  ...
  const postUserData = async (useData) => {
    ...
    //   `https://60f2479f6d44f300177885e6.mockapi.io/users?user_type=${useData.user_type}&?page=${0}&limit=${5}`
    // );
    // result.data.reverse();
    //  const tempUser = {...users}
    // tempUser[useData.user_type] = result.data
    // setUsers(tempUser);
    // tempUser[useData.user_type].unshift(useData)

    users[useData.user_type].push(response.data)
    setUsers(users);
                        ...
                        <div class="col-sm-9">
                          ...
                          >
                            ...
                            {divisions.map((dataItem) => {
                              return (
                                <option
                                  value={dataItem.name + "-" + dataItem.isoCode}
                                >
                                  {dataItem.name}
                                </option>
                              );
